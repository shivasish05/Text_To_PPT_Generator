...
TITLE_FONT_SIZE = Pt(36)  # Larger title font
SLIDE_FONT_SIZE = Pt(24)  # Bigger text font
TEXT_BOX_WIDTH = Inches(8)
TEXT_BOX_HEIGHT = Inches(4)
SPACE_BETWEEN_SLIDES = Inches(0.5)

# Custom colors
BACKGROUND_COLOR = RGBColor(0, 140, 186)
...
def generate_slide_titles(topic, num_slides=5):
    ...
    for i in range(num_slides):
        ...
        response = openai.ChatCompletion.create(
            ...
            messages=[
                {"role": "system", "content": "You are a chatbot that generates presentation slides."},
...
def generate_slide_content(slide_title):
    ...
    response = openai.ChatCompletion.create(
        model="text-davinci-003",
        messages=[
            {"role": "system", "content": "You are a chatbot that generates presentation slide content."},
            {"role": "user", "content": user_message},
        ],
        max_tokens=160
    )
...
def create_presentation(topic, slide_titles, slide_contents):
    ...
    SLIDE_TRANSITION = pptx.enum.slide.XLBlinds
    ...
    # Create content slides with custom transitions and backgrounds
    slide_layout = prs.slide_layouts[5]
    for slide_title, slide_content in zip(slide_titles, slide_contents):
        slide = prs.slides.add_slide(slide_layout)
        
        # Set slide transition
        slide.transition.transition = SLIDE_TRANSITION
